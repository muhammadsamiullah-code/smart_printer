...
class PdfQrFileScreen extends StatefulWidget {
  final String title;
  final String icon;
  final Color color;
  const PdfQrFileScreen({
    super.key,
    required this.title,
    required this.icon,
    required this.color,
  ...
  @override
  State<PdfQrFileScreen> createState() => _PdfQrFileScreenState();
}

class _PdfQrFileScreenState extends State<PdfQrFileScreen> {
  List<File> pdfFiles = [];
  ...
  Future<void> loadFiles() async {
    ...
    final files = dir.listSync().where((file) {
      return file.path.contains("qr_pdf_") && file.path.endsWith(".pdf");
    }).toList();
  ...
  await adsProvider.showAdInterstitial();
    ...
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => CreateQRPdfScreen(title: widget.title)),
    );
  ...
  void deleteFile(File file) async {
    ...
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: TrText("rename_file")));
  }
