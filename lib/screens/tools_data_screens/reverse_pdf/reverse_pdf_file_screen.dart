...
import 'reverse_pdf.dart';

class ReversePdfFileScreen extends StatefulWidget {
  final String title;
  final String icon;
  final Color color;
  const ReversePdfFileScreen({
    super.key,
    required this.title,
    required this.icon,
    required this.color,
  ...
  @override
  State<ReversePdfFileScreen> createState() => _ReversePdfFileScreenState();
}

class _ReversePdfFileScreenState extends State<ReversePdfFileScreen> {
  List<File> pdfFiles = [];
  ...
  Future<void> loadFiles() async {
    ...
    final files = dir.listSync().where((file) {
      return file.path.contains("reversed_") && file.path.endsWith(".pdf");
    }).toList();
    ...
    if (result != null) {
      ...
      if (files.isNotEmpty) {
        final updated = await Navigator.push(
          ...
          MaterialPageRoute(
            builder: (_) =>
                ReversePdfScreen(selectedFiles: files, title: widget.title),
          ),
        );
